...
class Network {
    ...
    initiateNeuralNetwork() {
        // Use params for min/max values
        const { minWeightValue, maxWeightValue, minBiasValue, maxBiasValue, minNValue, maxNValue } = this.params;

        for (let i = 1; i < this.layers.length; i++) {
            for (const connection of this.layers[i].connections) {
                 connection.weight = ToolBox.lerp(minWeightValue, maxWeightValue, Math.random());
            }
        }
        ...
        for (let i = 1; i < this.layers.length; i++) {
            ...
            for (let j = 0; j < thisLayer.neurons.length; j++) {
                ...
                for (let k = 0; k < prevLayer.neurons.length; k++) {
                    ...
                    sum += prevLayer.neurons[k].output * connection.weight;
                }

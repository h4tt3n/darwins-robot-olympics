...
class AngularState extends LinearState {
    ...
    applyImpulseAtPoint(impulse, point) {
        if (impulse instanceof Vector2 && point instanceof Vector2) {
            ...
            const linearImpulse = impulse.mul(this.inverseMass);
            const angularImpulse = r.perpDot(impulse) * this.inverseInertia;
    ...
    computeAngularVelocityVector(){
        const deltaAngle = this.angularVelocity * constants.DT;

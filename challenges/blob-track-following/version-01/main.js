...
function update() {
    ...
    simulation.trackFollowingBlobs.forEach(b => { b.followTrack(simulation.world.lineSegments)});
    simulation.update();
    evaluate();
}
...
function evaluate() {
    //
    for (let i = 0; i < simulation.trackFollowingBlobs.length; i++) {
        let blob = simulation.trackFollowingBlobs[i];
        if (blob.timeouts(1000) || blob.touchesWall()) {
            blob.calculateFitness();
            console.log("fitness: " + blob.fitness);
            simulation.deleteTrackFollowingBlob(blob);
        } else {
            blob.ticksAlive++;
            blob.updateFitness();
        }
    ...
    if (simulation.trackFollowingBlobs.length === 0) {
        
        simulation.deadIndividuals = [];

        for (let i = 0; i < simulation.deadTrackFollowingBlobs.length; i++) {
            let individualParams = {
                genome : simulation.deadTrackFollowingBlobs[i].brain.encode(),
                fitness : simulation.deadTrackFollowingBlobs[i].fitness,
            }
        ...
        for (let i = 0; i < simulation.individuals.length; i++) {
            ...
            simulation.createTrackFollowingBlob(blobParams);
        }
        
        simulation.deadTrackFollowingBlobs = [];
